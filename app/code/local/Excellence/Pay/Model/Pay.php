...
class Excellence_Pay_Model_Pay extends Mage_Payment_Model_Method_Cc
{
	...
	{
		//Mage::throwException(Mage::helper('paygate')->__("fasdfasdf"));
		$order = $payment->getOrder();
		$result = $this->callApi($payment,$amount,'authorize');
		if($result === false) {
			$errorCode = 'Invalid Data';
			...
			if($result['status'] == 1){
				...
				*/
				...
				Mage::throwException(Mage::helper('paygate')->__("Credit card decline."));
			}

			// Add the comment and save the order
		}
		if($errorMsg){
			Mage::throwException(Mage::helper('paygate')->__($errorMsg));
		}
